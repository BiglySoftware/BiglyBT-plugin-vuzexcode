...
public abstract class IPod extends DeviceSpecification {
...
//		return isVideoCompatible(info);
	...
	private boolean isVideoCompatible(MediaInformation info) {
		...
		if(stream.checkFormat(VideoFormat.H264)) {
			...
			//H.264 video, up to 2.5 Mbps, 640 by 480 pixels, 30 frames per second, Baseline Profile up to Level 3.0
			...
			if(!stream.checkMPEGProfile(3.0, MPEGProfile.BASELINE)) {
				return false;
			}
